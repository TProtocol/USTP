...
contract StbtTimelockController is TimelockController {
	...
	constructor(
		...
		require(
			selectors.length == delays.length,
			"TimelockController: SELECTORS_DELAYS_LEN_NOT_MATCH"
		);
...
contract STBT is Ownable, ISTBT {
	...
	function distributeInterests(
		...
		if (_distributedInterest > 0) {
			require(
				oldTotalSupply * maxDistributeRatio >= uint(_distributedInterest) * (10 ** 18),
				"STBT: MAX_DISTRIBUTE_RATIO_EXCEEDED"
			);
		...
		require(
			lastDistributeTime + minDistributeInterval < block.timestamp,
			"STBT: MIN_DISTRIBUTE_INTERVAL_VIOLATED"
		);
		emit InterestsDistributed(
	...
	function _checkTransfer(
		...
		uint256 _amount,
		bytes calldata /*_data*/
	) internal view returns (bool, uint8, bytes32) {
		Permission memory permTx = permissions[_sender];
		Permission memory permRx = permissions[_recipient];
		bool txOK = permTx.sendAllowed &&
			(permTx.expiryTime == 0 || permTx.expiryTime > block.timestamp) &&
			_sender != address(0);
		if (!txOK) {
			return (false, PermissionRequested, bytes32(bytes("CANNOT_SEND")));
		}
		bool rxOK = permRx.receiveAllowed &&
			(permRx.expiryTime == 0 || permRx.expiryTime > block.timestamp) &&
